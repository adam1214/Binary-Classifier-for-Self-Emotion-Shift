...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', "--random_num", type=int, help="select random number?", default=1111)
    ...
    feat_pooled = {}
    for val_ in val:
        feat_pooled_audio = joblib.load('./data/wav2vec2_iemocap_emo/wav2vec2_feat_z_norm_fold' + val_[-1] + '.pkl')
        feat_pooled_text = joblib.load('./data/roberta_iemocap_emo/roberta_feat_bos_fold' + val_[-1] + '.pkl')
        ...
        clf = make_pipeline(SVC(kernel='poly', random_state=args.random_num, probability=True))
    ...
    joblib.dump(pred_prob_dict, './output/SVM_emo_shift_output_tmp_text_audio.pkl')
    '''
    path = 'uar.txt'
    f = open(path, 'a')
