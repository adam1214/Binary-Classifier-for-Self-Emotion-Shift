...
if __name__ == '__main__':
    ...
    emo_shift_all = joblib.load('../data/4emo_shift_all.pkl')
    iaan_emo_shift = joblib.load('./demo_model/iaan_emo_shift_output.pkl')
    ...
    emo_num_map = {'Anger':0, 'Happiness':1, 'Neutral':2, 'Sadness':3}
    ...
    preds, labels = [], []
    for utt in emo_all:
        if emo_all[utt] in ['Anger', 'Happiness', 'Neutral', 'Sadness']:
    ...
    for name, model_prob, model_performance in zip(name_list, model_prob_list, model_performance_list):
        ...
        plt.text(x=-1, y=0.15 , s=model_performance)
