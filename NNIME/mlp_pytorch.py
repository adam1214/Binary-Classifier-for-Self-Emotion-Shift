...
class binaryClassification(nn.Module):
    def __init__(self, feat_dim):
        ...
        self.neuron_cnt = 600
        self.layer_1 = nn.Linear(in_features=feat_dim*2, out_features=self.neuron_cnt, bias=True)
        self.layer_2 = nn.Linear(in_features=self.neuron_cnt, out_features=self.neuron_cnt, bias=True)
        self.layer_out = nn.Linear(in_features=self.neuron_cnt, out_features=1, bias=True)
        
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=0.2, inplace=False)
        self.batchnorm1 = nn.BatchNorm1d(self.neuron_cnt)
        self.batchnorm2 = nn.BatchNorm1d(self.neuron_cnt)
...
def gen_train_val_test(data_frame, X, Y, utt_name=None):
    for index, row in data_frame.iterrows():
        ...
        center_utt_feat = dag_utt_embed_dict[center_utt_name]
        target_utt_feat = dag_utt_embed_dict[target_utt_name]
        oppo_utt_feat = dag_utt_embed_dict[oppo_utt_name]
        ...
        '''
        if utt_name != None: # train & test & val
            X.append([])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', "--fold_num", type=str, help="select fold number?", default='5')
    ...
    for test_, val_ in zip(test, val):
        ...
        '''
        dag_utt_embed_dict = joblib.load('./data/dag_H/text/fold' + test_[-1] + '/outputs_utt_embed_text.pkl')
        ...
        model = binaryClassification(feat_dim)
        model.to(device)
        ...
        generate_interaction_data(dialog_dict, dag_utt_embed_dict, emo_all_dict, test_set=test_, val_set=val_)
        ...
        for e in range(1, EPOCH+1, 1):
            ...
            val_acc = accuracy_score(y_gt_list_validation, y_pred_list_validation)*100
            ...
            if val_acc > best_performance or e == 1:
                ...
                torch.save(checkpoint, './model/mlp_pytorch_best_model_fold' + args.fold_num + '.pth')
        ...
        model = binaryClassification(feat_dim)
        model.to(device)
        checkpoint = torch.load('./model/mlp_pytorch_best_model_fold' + args.fold_num + '.pth')
        model.load_state_dict(checkpoint['model_state_dict'])
        model.eval()
        model_pred_and_gt(None, None, train_loader, model, pred_prob_dict=pred_prob_dict)
        model_pred_and_gt(None, None, val_loader, model, pred_prob_dict=pred_prob_dict)
        model_pred_and_gt(pred, gt, test_loader, model, pred_prob_dict=pred_prob_dict)
        joblib.dump(pred_prob_dict, './output/MLPPytorch_emo_shift_output_text_fold' + args.fold_num + '.pkl')
        '''
        if test_ == 'Ses01':
    ...
    joblib.dump(pred_prob_dict_fold1, './output/MLPPytorch_emo_shift_output_text_audio_fold1.pkl')
    joblib.dump(pred_prob_dict_fold2, './output/MLPPytorch_emo_shift_output_text_audio_fold2.pkl')
    joblib.dump(pred_prob_dict_fold3, './output/MLPPytorch_emo_shift_output_text_audio_fold3.pkl')
    joblib.dump(pred_prob_dict_fold4, './output/MLPPytorch_emo_shift_output_text_audio_fold4.pkl')
    joblib.dump(pred_prob_dict_fold5, './output/MLPPytorch_emo_shift_output_text_audio_fold5.pkl')
    '''
